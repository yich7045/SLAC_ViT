...
class SlacAlgorithm:
    ...
    def preprocess(self, ob, Data_Aug):
    ...
    def prepare_batch(self, state_, tactile_, action_):
        with torch.no_grad():
            # f(1:t+1)
            feature_ = self.latent.encoder(state_, Data_Augmentation=True)
            # z(1:t+1)
